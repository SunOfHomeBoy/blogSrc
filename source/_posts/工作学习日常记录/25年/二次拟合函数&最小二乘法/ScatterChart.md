...
const ScatterChart: React.FC<ScatterChartProps> = ({ direction }) => {
  ...
  const [fittedCurve, setFittedCurve] = useState<[number, number][]>([])
  /** 流量最大值点 State */
  const [maxFlowPoint, setMaxFlowPoint] = useState<[number, number] | null>(null)
  const [, /* _modelParams */ setModelParams] = useState({})
      ...
      if (pointsInArea.length > 0) {
        ...
        subSaturationArea = [
          [
            {
              name: '亚饱和区域',
              xAxis: min[0], // 起始X：最大流量点的流量
              yAxis: min[1], // 起始Y：区域内最小速度
            },
            {
              xAxis: max[0], // 结束X：图表X轴最大值
              yAxis: max[1], // 结束Y：区域内最大速度
            },
          ],
        ]
      }
      ...
      ? [
          {
            ...
            label: {
              formatter: `${maxFlowPoint[1].toFixed(2)}km/h`,
              position: 'end',
            },
          },
          {
            ...
            label: {
              formatter: `${maxFlowPoint[0].toFixed(0)}辆/10分钟`,
              position: 'end',
            },
          },
        ]
      : [] // 如果 maxFlowPoint 为 null，则不显示标线
  ...
  }, [fittedCurve, direction, scatterData, maxFlowPoint])
  ...
  const fitQuadratic = (dataList: [number, number][]) => {
    ...
    // 对曲线数据按流量排序，确保绘制时是连续的
    // curveData.sort((p1, p2) => p1[0] - p2[0]) // 注释掉此行以修复曲线绘制顺序问题

    /** 找到流量最大点 */
    let maxFlowPoint: [number, number] | null = null
  ...
  useEffect(() => {
    ...
    setFittedCurve(fitted.curve)
    setModelParams(fitted.params)
    setMaxFlowPoint(fitted?.maxFlowPoint)
  }, [scatterData])

  /* 初始化图表逻辑 */
  useEffect(() => {
    /** 非空校验 确保有足够的数据点、拟合曲线已生成，并且最大流量点已计算出来 */
    if (scatterData.length < 3 || fittedCurve.length === 0 || !maxFlowPoint) return

    const cleanup = initChart()
    return () => cleanup && cleanup()
  }, [initChart, scatterData, fittedCurve, maxFlowPoint])
