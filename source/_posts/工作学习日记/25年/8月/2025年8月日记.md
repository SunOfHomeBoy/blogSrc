 ...
 * @LastEditTime: 2025-08-29 14:34:50
...
## 申请免费 带有公网IP服务器
  + 国内云服务提供商
    + 华为云：华为云为新用户提供 12 个月的 ECS（弹性云服务器）实例免费试用，包括 1 个标准型实例、1 个云硬盘和 1 个公网 IP，申请时需完成实名认证。
    + 阿里云：阿里云新用户可以免费试用一台 1 核 2GB 内存的云服务器，使用时长为一年，每个月有约 40GB 的数据传输免费额度，同样需要完成实名认证才能申请。
    + 腾讯云：腾讯云为新用户提供 1 个月的 CVM（云服务器）免费试用，包括 1 个 1 核 1GB 内存的虚拟机实例、1 个 50GB 的云硬盘和 1 个免费的公网 IP，申请流程中需完成实名认证。
  + 国外云服务提供商
    + Google Cloud Platform（GCP）：GCP 免费层为新用户提供免费的计算资源和 IP 地址，适合进行开发和测试。
    + Amazon Web Services（AWS）：AWS 免费层为用户提供每月一定额度的免费服务器资源，包括 EC2 实例和 IP 地址。
    + Oracle Cloud Free Tier：提供免费的虚拟机和公网 IP，可以搭建多种小型应用。
