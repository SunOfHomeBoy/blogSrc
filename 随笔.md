...
# MongoDB学习笔记(JS胖)
 (2018年1月26日 13:53:11)(http://jspang.com/2017/12/16/mongdb/)
## 第01节：认识和安装MongoDB
   > MongoDB是`非关系型数据库`
...
## 第02节：Mongo基本命令-1(`show dbs` `use xxdb` `show collections` `db` `MongoDB的存储结构`)
   接下来的几节直接上手Mongo命令,MSSQL和MYsql用的都是Sql命令。
   ...
   **基础Shell命令**
   ...
   `总结`：这节 学习已经 进入 `MongoDB` 世界,感觉用起 比较简单,都是 基本命令, 敲回车就可使用,但还要进行多 练习。`重点` 要转变 以前`数据库存储结构的思想`,`掌握MongoDB的存储结构`。 
...
## 第05节：批量插入的正确方法("批量数据插入 以`数组`方式" `老版本MongoDB batchInsert` `一次插入不要超 48M` `静态存储` `批量插入性能测试 批量>循环` `数据库性能`)
   ...
   **批量插入**
    ...
    `db.test.batchInsert()`
    **注意**一次插入不要超过48M,.zip和大图片什么的尽量用静态存储,MongoDB存储静态路径就好,这也算是一个规则。
